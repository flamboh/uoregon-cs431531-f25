...
int main(int argc, char **argv)
{
    ...
    t0 = ReadTSC();
    allocate_ell_gpu(ell_col_ind, ell_vals, m, n_new, n, nnz, x, &dec, &dev, &dex,
                     &deb);
    timer[GPU_ALLOC_TIME] += ElapsedTime(ReadTSC() - t0);
                        ...
                        int *n_new)
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        if (row_nnz > max_nnz_in_row)
        {
            max_nnz_in_row = row_nnz;
        }
    }
    *n_new = max_nnz_in_row;

    *ell_col_ind = (unsigned int *)malloc(sizeof(unsigned int) * m * max_nnz_in_row);
    assert(*ell_col_ind);
    *ell_vals = (double *)malloc(sizeof(double) * m * max_nnz_in_row);
    assert(*ell_vals);
